...
export default {
  ...
  methods: {
    async downloadWeight() {
      ...
      const weightIndex = this.selectedModel.weights.length - 1;

      const weight = {
        name: this.selectedModel.weights[weightIndex].fileName,
        model: this.selectedModel.name,
        filePath: this.selectedModel.weights[weightIndex].filePath,
        model_id: this.selectedModel._id,
        weight_id: this.selectedModel.weights[weightIndex]._id,
      };
      ...
      Vue.set(this.selectedModel.weights[weightIndex], "downloadPath", newPath);
      this.models.forEach((m) => {
        if (m._id === this.selectedModel._id) {
          m.weights[
            this.selectedModel.weights.length - 1
          ].downloadPath = newPath;
        }
      });
      await AIModelActions.saveModels(this.models);
