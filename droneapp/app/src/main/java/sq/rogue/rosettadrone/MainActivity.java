...
//        AlertDialog.Builder alertDialog2 = new AlertDialog.Builder(this, R.style.CustomDialog);
   ...
   //     alertDialog2.setBackgroundDrawableResource(android.R.color.transparent);
        alertDialog2.setNegativeButton("Finding...",
                (dialog, which) -> {
                    mModel.setAIfunction(3);
                    r.stop();
                    dialog.cancel();
                });
        alertDialog2.setNeutralButton("Tracking...",
                (dialog, which) -> {
                    mModel.setAIfunction(2);
                    r.stop();
                    dialog.cancel();
                });
        alertDialog2.setPositiveButton ("Inspecting...",
                (dialog, which) -> {
            ...
            if (m_videoMode == 2) {
                ...
                if(mExternalVideoOut == true)
                    try {
                        ...
                        logMessageDJI("Error NativeHelper.getInstance().parse(videoBuffer, size,0);");
                        Log.e(TAG, "Error NativeHelper.getInstance().parse(videoBuffer, size,0);");
                    }
            } else {
                ...
                if(mExternalVideoOut == true)
                    try {
                        ...
                        logMessageDJI("Error NativeHelper.getInstance().parse(videoBuffer, size,1);");
                        Log.e(TAG, "Error NativeHelper.getInstance().parse(videoBuffer, size,1);");
                    }
            }
        };
...
*/
                ...
                logMessageDJI("UNKOWN AIRCRAFT ERROR");
            }
        }
    }
...
*/
            ...
            videostreamPreviewTtView.setVisibility(View.VISIBLE);
            videostreamPreviewTtView.requestFocus(); // .buildLayer();
